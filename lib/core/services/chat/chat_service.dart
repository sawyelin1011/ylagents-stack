...
class ChatService extends ChangeNotifier {
  ...
  /// Get conversations filtered by workspace. If workspaceId is null,
  /// returns all conversations (for backward compatibility).
  List<Conversation> getConversationsForWorkspace(String? workspaceId) {
    final all = getAllConversations();
    if (workspaceId == null) return all;
    return all
        .where((c) => c.workspaceId == null || c.workspaceId == workspaceId)
        .toList();
  }

  /// Assign all conversations with null workspaceId to the given workspace.
  Future<void> assignUnassignedConversationsToWorkspace(
    String workspaceId,
  ) async {
    if (!_initialized) await init();
