...
bool _didEnsureWorkspace = false; // ensure personal workspace after l10n ready
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProvider(
          create: (ctx) => AgentProvider(ctx.read<AssistantProvider>()),
        ),
                ...
                builder: (ctx, child) {
                  ...
                  if (!_didEnsureAssistants) {
                    ...
                    WidgetsBinding.instance.addPostFrameCallback((_) async {
                      ...
                      // Ensure Personal workspace exists + migrate legacy data
                      if (!_didEnsureWorkspace) {
                        _didEnsureWorkspace = true;
                        try {
                          final wsProvider = ctx.read<WorkspaceProvider>();
                          await wsProvider.ensurePersonalWorkspace(
                            AppLocalizations.of(
                              ctx,
                            )!.workspaceProviderPersonalWorkspaceName,
                          );
